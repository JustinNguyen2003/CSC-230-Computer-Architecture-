 ...
 * Student UVic ID: V00978867
 * Date of completed work: December 1st, 2021
 *
 *
...
void led_state(uint8_t LED, uint8_t state) {
	DDRL = 0xFF;
	int setBit = 128 >> 2*LED;
	int checkBit = PORTL & setBit;
	if (state == 1 && checkBit != setBit){   /*checks if bit being set is already on */
		PORTL |= setBit;
	}
	else if(state == 1 && checkBit == setBit){
		return;
	}
	else if(state == 0 && checkBit == setBit){
		PORTL ^= setBit;
	}
	else{
		return;
...
void SOS() {
	...
	for (int i=0; i<length; i++){
		if (light[i]== 0x1){
			int setLight = 0;
			led_state(setLight, 1);
			_delay_ms(duration[i]);
			led_state(setLight, 0);
		}
		else if (light[i] == 0xf){
			led_state(0, 1);
			led_state(1, 1);
...
void glow(uint8_t LED, float brightness) {
	...
	for( ;; ){
		...
		if (count < threshold && temp == 0){
			led_state(LED, 1);
		}
		
		else if(count<PWM_PERIOD && temp != 0){
			led_state(LED, 0);
		}
...
void pulse_glow(uint8_t LED) {
	...
	for( ;; ){
		...
		if (check_bright == 0 && slow_count>9){
			threshold--;
			slow_count = 0;
		}
		if (check_bright == 1 && slow_count>9){
			threshold++;
			slow_count =0;
		}
